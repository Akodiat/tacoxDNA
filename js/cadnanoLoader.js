...
function vhelix_rotation_origami_sq(direction, perp) {
    return perp.clone().applyAxisAngle(direction, Math.PI * 15. / 180);
}
function vhelix_rotation_origami_he(direction, perp) {
    return perp.clone().applyAxisAngle(direction, Math.PI * 160. / 180);
}
function insert_loop_skip(start_pos, direction, perp, rot, helix_angles, vhelix, nodes, use_seq, seqs) {
    ...
    let new_angle = [];
    let helix_angles_new = helix_angles.slice(); // copy
    let reverse_nodes;
    if (vhelix.num % 2 === 1) {
...
function add_slice(current_system, vhelix, begin, end, nodes, strands, pos, direction, perp, rot, helix_angles, strand_type, use_seq, seqs) {
...
function add_slice_nupack(vhelix, strand_number, begin_helix, end_helix, index_lookup, strand_type) {
    let length_change = 0;
    let skips = 0;
    let loops = 0;
    if ((vhelix.num % 2 + strand_type) % 2 === 0) { //  strand and even num or staple and odd num
...
function build_nodes(vh) {
    ...
    for (let i = 0; i < vh.scaf.length; i++) {
        ...
        if (!((prev === prev_stap && (prev === 'begin' || prev === 'end')) ||
...
function generate_vhelices_origami_sq(vhelix_direction, vhelix_perp, h) {
    ...
    let pos, direction, perp, rot, angles;
    if (h.num % 2 === 0) {
...
function generate_vhelices_origami_he(vhelix_direction, vhelix_perp, h) {
    let g = new cu.StrandGenerator();
    //  generate helix angles
    let helix_angles = new Array(h.len - 1);
    for (let i = 0; i < helix_angles.length; i++) {
    ...
    //  make the virtual helices
    let pos, direction, perp, rot, strands;
    if (h.num % 2 === 0) {
...
function parse_cadnano(json_string) {
    ...
    for (const vstrand of cadnano["vstrands"]) {
        ...
        for (const [key, val] of Object.entries(vstrand)) {
            if (key === "skip") {
                vh.skip = val.map(x => Math.abs(x));
            }
            else if (key === "stap" || key === "scaf") {
                vh[key] = val.map(x => new square(...x));
            }
            else {
                vh[key] = val;
            }
