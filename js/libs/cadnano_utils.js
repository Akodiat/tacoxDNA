...
class StrandGenerator {
    generate(bp, sequence = undefined, start_pos = new THREE.Vector3(0, 0, 0), direction = new THREE.Vector3(0, 0, 1), perp = undefined, rot = 0., double = true, circular = false, DELTA_LK = 0, BP_PER_TURN = 10.34, ds_start = undefined, ds_end = undefined, force_helicity = false) {
        ...
        */
        ...
        if (circular && option_use_helicity) {
            ...
            for (let i = 0; i < bp; i++) {
                rb = new THREE.Vector3(Math.cos(i * angle) * radius + 0.34 * Math.cos(i * angle), Math.sin(i * angle) * radius + 0.34 * Math.sin(i * angle), 0);
                a1 = new THREE.Vector3(Math.cos(i * angle), Math.sin(i * angle), 0);
    ...
    generate_or_sq(bp, sequence = undefined, start_pos = new THREE.Vector3(0., 0., 0.), direction = new THREE.Vector3(0., 0., 1.), perp = undefined, double = true, rot = 0., angle = Math.PI / 180 * 33.75, length_change = [], region_begin = [], region_end = []) {
        ...
        if (double) {
            ...
            for (let i = 0; i < bp; i++) {
                ...
                if (i != bp - 1) {
                    //  we loop over the quaternions in the reverse order, and use the inverse of rotation
                    a1.applyQuaternion(Rs.pop().invert()).normalize();
    ...
    generate_double_offset(seqA, seqB, offset, start_pos = new THREE.Vector3(0, 0, 0), direction = new THREE.Vector3(0, 0, 1), perp = undefined, rot = 0) {
        let seqa;
...
class pairMap {
    ...
    *entries() {
        for (let [k, v] of this.map.entries()) {
            let keyPair = k.split(',').map(v => parseInt(v));
            yield [keyPair, v];
        }
    }
    values() {
